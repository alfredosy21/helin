...
class PermissionMiddleware
{
    ...
    /**
     * Check if user has permission for the specified module by name or id.
     */
    private function hasModulePermissionByName(int $roleId, string $moduleName): bool
    {
        $module = is_numeric($moduleName)
                ? Module::where('id', (int) $moduleName)->first()
    ...
    /**
     * Check if user has permission for the specified submodule by name or id.
     */
    private function hasSubmodulePermissionByName(int $roleId, string $submoduleName): bool
    {
        $submodule = is_numeric($submoduleName)
                ? Submodule::where('id', (int) $submoduleName)->first()
                : Submodule::where('name', $submoduleName)->first();
