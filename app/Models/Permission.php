...
class Permission extends Model
{
    ...
    public static function createPermissions($role_id)
    {
        try {
            ...
            foreach (Module::all() as $rs) {
                $existing = Permission::where('rol_id', $role_id)
                    ->where('module_id', $rs->id)
                    ->whereNull('submodule_id')
                    ->where('type', self::MAIN_MODULE_TYPE)
                    ->first();

                if ($existing) {
                    $existing->update(['status' => self::ACTIVE_STATUS]);
                } else {
                    Permission::create([
                        'rol_id' => $role_id,
                        'module_id' => $rs->id,
    ...
    {
        $permissions_from_modules = Permission::join('modules', 'modules.id', '=', 'permissions.module_id')
        ...
        foreach ($permissions_from_modules as $rs) {
            $submodules = [];
            $permissions_from_submodules = Permission::join('submodules', 'submodules.id', '=', 'permissions.submodule_id')
                ->where('permissions.rol_id', $role_id)
