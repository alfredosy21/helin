...
- **`Sections::$fillable` incompleto**: Añadir `subtitle`, `description`, `items`, `buttons`, `layout_type`, `icon_style` — columnas **ya existentes en BD** (migración `2026_06_27_120000`) pero no expuestas.
...
- **`Resource::$fillable` incorrecto**: Incluye `specialty`/`tags` (no existen como columnas); NO incluye `content`, `diagnosis`, `gallery`, `video_url` (requieren migración — fase 3).
- **`ResourceController` filtra por columna inexistente**: `filterSpecialty` filtra por `specialty` (string) en lugar de `resource_specialty_id` (FK). Corregir.
...
- **Materiales y resultados de caso clínico**: Reemplazar los 5 materiales estáticos por `$resource->materials` y el texto de resultados por `$resource->results`.
