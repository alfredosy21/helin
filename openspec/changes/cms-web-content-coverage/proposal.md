...
- **`Resource::$fillable` incorrecto**: Incluye `specialty`/`tags` (no existen como columnas); NO incluye `slug` (que existe en BD y se usa en rutas/vistas), ni `content`, `diagnosis`, `gallery`, `video_url` (requieren migración — fase 3).
...
- **`WebController::recursosClinicos()` duplica recursos artificialmente**: Líneas 358-359 multiplican los recursos ×4 (`array_fill(0, $multiplier, $r)`) para simular más contenido. Esto es un bug de diseño que infla la paginación y muestra recursos repetidos. Eliminar el multiplicador y paginar los recursos reales.
- **`WebController::searchProducts()` usa imágenes hardcoded**: Líneas 403-405 usan un pool estático `['im1.png'...'im6.png']` para los resultados de búsqueda AJAX en lugar de las imágenes reales del producto. Reemplazar por `Product::mainImageUrl` o equivalente.
...
- **`settings`**: `opinion_url` (string nullable), `offices` (JSON nullable — reemplaza los campos individuales `caracas_location`/`valencia_location`/`barquisimeto_location` con estructura `[{name, url, active}]`).
...
- **Sedes en contacto**: Los nombres de sedes (Caracas, Valencia, Barquisimeto) están hardcodeados en `contactanos.blade.php` aunque las URLs vienen de Settings. **Decisión**: reemplazar los campos individuales `caracas_location`/`valencia_location`/`barquisimeto_location` por un campo JSON `offices` en `Settings` con estructura `[{name, url, active}]`, gestionable desde el CMS. La vista itera el JSON en lugar de tener 3 bloques hardcodeados.
...
- **Migraciones**: **Sin migración para `sections` ni `products`** (las columnas ya existen, solo falta actualizar `$fillable`). Migraciones nuevas para:
  ...
  - `settings.offices`: añadir columna JSON `offices` (reemplaza `caracas_location`/`valencia_location`/`barquisimeto_location`)
