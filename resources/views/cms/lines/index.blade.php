<div class="p-6 lg:p-8 bg-gray-50 min-h-screen">
    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Líneas de Producto</h1>
            <p class="text-sm text-gray-500 mt-1">Gestiona la clasificación principal del catálogo y orden de visualización.</p>
        </div>
            ...
            <table class="w-full text-sm text-left text-gray-500">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-4 font-semibold">Línea</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($lines as $line)
                        <tr wire:key="line-{{ $line->id }}" class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end items-center gap-2">
                                    <button wire:click="edit({{ $line->id }})" class="p-2 text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Editar Línea">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                        </svg>
                                    </button>
                                    <button wire:confirm="¿Deseas eliminar esta línea? Se podrían afectar categorías y productos asociados." wire:click="confirmDelete({{ $line->id }})" class="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Eliminar Línea">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center text-gray-400 font-medium">
                                No se encontraron líneas de producto en el inventario.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg transform transition-all overflow-hidden">
            <div class="p-6 border-b border-gray-100 flex justify-between items-center">
                <h3 class="text-xl font-bold text-gray-900">
                    {{ $editingId ? 'Actualizar Línea' : 'Nueva Línea de Catálogo' }}
                </h3>
            ...
            <form wire:submit.prevent="save">
                <div class="p-6 space-y-5">
                    <!-- Nombre -->
                    <div>
                        <label class="block text-xs font-bold text-gray-700 uppercase mb-2 tracking-wider">Nombre de la Línea</label>
                        <input type="text" wire:model="name" 
                            ...
                            placeholder="ej. Equipamiento Médico, Instrumental, Consumibles...">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="slug" 
                                ...
                                placeholder="ej. equipamiento-medico">
