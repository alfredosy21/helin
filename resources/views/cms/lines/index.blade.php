<div class="min-h-screen pb-12 bg-soft relative">
    ...
    <div class="relative z-10 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-6">
        ...
        <x-ui-section-header :module-id="\App\Models\Module::CATALOG" :submodule-id="\App\Models\Submodule::PRODUCT_LINES" :subtitle="__('cms.lines.title')">
            @if(!$showForm)
            <x-slot:action>
                ...
                <table class="w-full table-fixed text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/60 text-[10px] font-semibold text-body uppercase tracking-wider border-b border-slate-100">
                            <th class="px-4 py-2.5">{{ __('cms.tables.line') }}</th>
                    ...
                    <tbody id="lines-table-body" class="divide-y divide-slate-100 text-[13px]">
                        @forelse($lines as $line)
                        <tr wire:key="line-{{ $line->id }}" data-id="{{ $line->id }}" class="sortable-row hover:bg-slate-50/70 transition-colors duration-150">
                            ...
                            <td class="px-4 py-2.5 text-center">
                                <x-ui-badge-status :active="$line->is_active" :active-label="__('cms.general.status_active')" :inactive-label="__('cms.general.status_inactive')" />
                            </td>
                            <td class="px-4 py-2.5 text-right">
                                <div class="flex justify-end gap-1">
                                    <x-cms-tooltip text="{{ __('cms.general.edit') }}">
                                        <button wire:click="edit({{ $line->id }})" class="p-2 text-body hover:text-primary hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                    ...
                                    <x-cms-tooltip text="{{ __('cms.general.delete') }}">
                                        <button onclick="deleteLine({{ $line->id }})" class="p-2 text-body hover:text-red-500 hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        @empty
                        <tr>
                            <td colspan="4" class="px-4 py-8 text-center"><x-ui-empty-state icon="folder" :title="__('cms.lines.no_lines')" />
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <form wire:submit.prevent="save" class="space-y-4 sm:space-y-5">
            ...
            {{-- Información básica --}}
            <x-ui-form-card title="{{ $editingId ? __('cms.lines.edit_title') : __('cms.lines.new_title') }}" description="{{ __('cms.lines.subtitle') }}" icon="layers">
                <div class="space-y-4">
                    <x-ui-toggle wire:model="is_active" :label="__('cms.general.status_active')" />

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                        <div class="space-y-1.5">
                            <label class="text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.lines.name_label') }} <span class="text-red-500">*</span></label>
                        ...
                        <div class="space-y-1.5">
                            <label class="text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.lines.slug_label') }}</label>
            ...
            {{-- Banner --}}
            <x-ui-form-card title="Banner" description="Imagen promocional de la línea" icon="image">
                <div class="space-y-4">
                    <div class="space-y-1.5">
