<div class="min-h-screen pb-12 bg-[#f8fafc] relative">
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody id="lines-table-body" class="divide-y divide-slate-50 text-sm">
                        @forelse($lines as $line)
                            <tr wire:key="line-{{ $line->id }}" data-id="{{ $line->id }}" class="hover:bg-slate-50/50 transition-colors">
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-1">
                                        ...
                                        <x-cms-tooltip text="{{ __('cms.general.delete') }}">
                                            <button onclick="openDeleteModal({{ $line->id }})" class="p-2 text-slate-400 hover:text-red-500 hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    ...
    <script>
        ...
        function openDeleteModal(lineId) {
            deleteLineId = lineId;
        ...
        function confirmDelete() {
            if (deleteLineId) {
                Livewire.find('{{ $this->getId() }}').confirmDelete(deleteLineId);
                closeDeleteModal();
            }
        }
    </script>
