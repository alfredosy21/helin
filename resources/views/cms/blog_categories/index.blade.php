<div class="min-h-screen pb-12 bg-[#f8fafc] relative">
    ...
    <div class="relative z-10 p-6 space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-2">
            <div>
                <x-cms-breadcrumb module="cms.blog_categories.breadcrumb">
            ...
            <button wire:click="create" class="rounded-lg bg-primary hover:bg-[#079d8b] text-white px-4 py-2.5 text-sm font-medium transition-colors inline-flex items-center shadow-none border-none cursor-pointer">
                ...
                {{ __('cms.blog_categories.new_button') }}
            </button>
        </div>
        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-[0_1px_2px_0_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="p-4 bg-white border-b border-slate-50 flex flex-col md:flex-row gap-3">
                <div class="relative flex-1">
                    ...
                    <input type="text" wire:model.live="search" placeholder="{{ __('cms.blog_categories.search_placeholder') }}"
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/70 border-b border-slate-100 text-[#c0c1c6] text-xs font-semibold">
                            ...
                            <th class="px-6 py-3.5 text-center w-24">{{ __('cms.tables.color') }}</th>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-1">
                                        <x-cms-tooltip text="{{ __('cms.general.edit') }}">
                                            <button wire:click="edit({{ $blogCategory->id }})" class="p-2 text-slate-400 hover:text-primary hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                        ...
                                        <x-cms-tooltip text="{{ __('cms.general.delete') }}">
                                            <button onclick="openDeleteModal({{ $blogCategory->id }})" class="p-2 text-slate-400 hover:text-red-500 hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <div class="relative w-full max-w-md h-full bg-white shadow-xl flex flex-col border-l border-slate-100">
            <div class="p-6 border-b border-slate-50 flex justify-between items-center">
                <div>
                    <h2 class="text-base font-bold text-[#222]">
                        {{ $editingId ? __('cms.blog_categories.edit_title') : __('cms.blog_categories.new_title') }}
                    </h2>
                    <p class="text-xs text-[#c0c1c6]">{{ __('cms.blog_categories.subtitle') }}</p>
                </div>
            ...
            <form wire:submit.prevent="save" class="flex flex-col flex-1 h-full">
                <div class="flex-1 overflow-y-auto p-6 space-y-5">
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.blog_categories.description_label') }}</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.blog_categories.seo_description_label') }}</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.blog_categories.slug_label') }}</label>
                        <input type="text" wire:model="slug" placeholder="{{ __('cms.blog_categories.slug_placeholder') }}"
                            class="w-full px-3 py-2 bg-slate-50 border border-slate-100 text-sm text-slate-700 rounded-lg focus:outline-none focus:border-primary transition-colors placeholder-slate-300" />
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.blog_categories.color_label') }}</label>
                        <div class="flex gap-2">
                            ...
                            <input type="text" wire:model="color"
                                ...
                                placeholder="{{ __('cms.blog_categories.color_placeholder') }}" pattern="^#[0-9A-Fa-f]{6}$">
                        </div>
                        @error('color') <span class="text-xs text-red-500 font-medium italic">{{ $message }}</span> @enderror
                        <p class="text-xs text-[#c0c1c6] italic">{{ __('cms.blog_categories.color_helper') }}</p>
                    </div>

                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.blog_categories.icon_label') }}</label>
                        <input type="text" wire:model="icon" placeholder="{{ __('cms.blog_categories.icon_placeholder') }}"
                            class="w-full px-3 py-2 bg-slate-50 border border-slate-100 text-sm text-slate-700 rounded-lg focus:outline-none focus:border-primary transition-colors placeholder-slate-300" />
                        @error('icon') <span class="text-xs text-red-500 font-medium italic">{{ $message }}</span> @enderror
                    </div>

                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.blog_categories.image_label') }}</label>
                        <input type="text" wire:model="image" placeholder="{{ __('cms.blog_categories.image_placeholder') }}"
                            class="w-full px-3 py-2 bg-slate-50 border border-slate-100 text-sm text-slate-700 rounded-lg focus:outline-none focus:border-primary transition-colors placeholder-slate-300" />
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        ...
        <div class="fixed inset-0 flex items-center justify-center p-4">
            <div class="relative bg-white rounded-xl shadow-xl w-full max-w-sm border border-slate-100 transform transition-all p-6 text-center">
                ...
                <h3 class="text-base font-bold text-[#222] mb-1">{{ __('cms.blog_categories.delete_title') }}</h3>
                <p class="text-xs text-slate-400 mb-6">{{ __('cms.general.delete_confirm_text_with_associated') }}</p>
                ...
                <div class="flex gap-3">
                    ...
                    <button onclick="confirmDelete()" class="flex-1 px-4 py-2 text-xs font-medium text-white bg-red-500 hover:bg-red-600 rounded-lg transition-colors border-none cursor-pointer">
                        {{ __('cms.blog_categories.delete_button') }}
                    </button>
                </div>
            </div>
        </div>
