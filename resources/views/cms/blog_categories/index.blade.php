<div class="min-h-screen pb-12 bg-soft relative">
    ...
    <div class="relative z-10 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-6">
        ...
        <x-ui-section-header :module-id="\App\Models\Module::BLOG" :submodule-id="\App\Models\Submodule::BLOG_CATEGORIES" :subtitle="__('cms.blog_categories.title')">
            @if(!$showForm)
            <x-slot:action>
                {{-- Botón Principal --}}
        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-[0_1px_2px_0_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody id="blog-categories-table-body" class="divide-y divide-slate-100 text-[13px]">
                        @forelse($blogCategories as $blogCategory)
                        <tr wire:key="blog-category-{{ $blogCategory->id }}" data-id="{{ $blogCategory->id }}" class="sortable-row hover:bg-slate-50/70 transition-colors duration-150">
                            ...
                            <td class="px-4 py-2.5 text-center">
                                <x-ui-badge-status :active="$blogCategory->is_active" :active-label="__('cms.general.status_active')" :inactive-label="__('cms.general.status_inactive')" />
                            </td>
                            <td class="px-4 py-2.5 text-right">
                                <div class="flex justify-end gap-1">
                                    <x-cms-tooltip text="{{ __('cms.general.edit') }}">
                                        <button wire:click="edit({{ $blogCategory->id }})" class="p-2 text-body hover:text-primary hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                    ...
                                    <x-cms-tooltip text="{{ __('cms.general.delete') }}">
                                        <button onclick="deleteBlogCategory({{ $blogCategory->id }})" class="p-2 text-body hover:text-red-500 hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        @empty
                        <tr>
                            <td colspan="4" class="px-4 py-8 text-center"><x-ui-empty-state icon="folder" title="No se encontraron categorías de blog" />
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="max-w-4xl mx-auto bg-white rounded-xl border border-slate-100 shadow-[0_1px_3px_0_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="p-4 sm:p-6 border-b border-slate-50">
                ...
                <p class="text-[13px] text-body mt-1">{{ __('cms.blog_categories.subtitle') }}</p>
            </div>
                ...
                <div class="p-4 sm:p-6 space-y-4 sm:space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        <div class="space-y-1.5">
                            <label class="text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.blog_categories.name_label') }} <span class="text-red-500">*</span></label>
                        ...
                        <div class="space-y-1.5">
                            <label class="text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.blog_categories.slug_label') }}</label>
                            ...
                            <p class="text-[13px] text-body italic mt-1">{{ __('cms.blog_categories.slug_helper') }}</p>
                        </div>
                    </div>

                    {{-- Textareas a ancho completo --}}
                    <div class="space-y-1.5">
                        <label class="text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.blog_categories.description_label') }}</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.blog_categories.seo_description_label') }}</label>
    ...
    <script>
        ...
        title: '{{ __('cms.blog_categories.delete_title') }}',
                text: '{{ __('cms.general.delete_confirm_text_with_associated') }}',
                confirmButtonText: '{{ __('cms.blog_categories.delete_button') }}',
                cancelButtonText: '{{ __('cms.general.cancel') }}',
                onConfirm: function() {
                Livewire.find('{{ $this->getId() }}').confirmDelete(blogCategoryId);
                }
        });
        }
