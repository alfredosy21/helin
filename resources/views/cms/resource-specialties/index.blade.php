<div class="min-h-screen pb-12 bg-soft relative">
    ...
    <div class="relative z-10 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-6">
                ...
                <table class="w-full table-fixed text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/60 text-[10px] font-semibold text-body uppercase tracking-wider border-b border-slate-100">
                            <th class="px-4 py-2.5">{{ __('cms.resource_specialties.specialty') }}</th>
                    ...
                    <tbody id="resource-specialties-table-body" class="divide-y divide-slate-100 text-[13px]">
                        @forelse($resourceSpecialties as $resourceSpecialty)
                        <tr wire:key="resource-specialty-{{ $resourceSpecialty->id }}" data-id="{{ $resourceSpecialty->id }}" class="sortable-row hover:bg-slate-50/70 transition-colors duration-150">
                            <td class="px-4 py-2.5">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <div>
                                            <div class="text-body block truncate">{{ $resourceSpecialty->name }}</div>
                                        </div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-2.5 text-center">
                                <x-ui-badge-status :active="$resourceSpecialty->is_active" :active-label="__('cms.general.status_active')" :inactive-label="__('cms.general.status_inactive')" />
                            </td>
                            <td class="px-4 py-2.5 text-right">
                                <div class="flex justify-end gap-1">
                                    <x-cms-tooltip text="{{ __('cms.general.edit') }}">
                                        <button wire:click="edit({{ $resourceSpecialty->id }})" class="p-2 text-body hover:text-primary hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                    ...
                                    <x-cms-tooltip text="{{ __('cms.general.delete') }}">
                                        <button onclick="deleteResourceSpecialty({{ $resourceSpecialty->id }})" class="p-2 text-body hover:text-red-500 hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        @empty
                        <tr>
                            <td colspan="4" class="px-4 py-8 text-center"><x-ui-empty-state icon="folder" :title="__('cms.resource_specialties.no_specialties')" />
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <form wire:submit.prevent="save" class="space-y-4 sm:space-y-5">
            ...
            {{-- Información básica --}}
            <x-ui-form-card title="{{ $editingId ? __('cms.resource_specialties.edit_title') : __('cms.resource_specialties.new_title') }}" description="{{ __('cms.resource_specialties.subtitle') }}" icon="stethoscope">
                <div class="space-y-4">
                    <x-ui-toggle wire:model="is_active" :label="__('cms.general.status_active')" />

                    <div class="space-y-1.5">
                        <label class="text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.resource_specialties.name_label') }} <span class="text-red-500">*</span></label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.resource_specialties.description_label') }}</label>
            ...
            {{-- Imagen --}}
            <x-ui-form-card title="Imagen" description="Imagen y banner de la especialidad" icon="image">
                <div class="space-y-4">
                    <div class="space-y-1.5">
                        <label class="text-[11px] font-semibold text-body uppercase tracking-wider">Imagen</label>
                        <div class="relative">
                            @if($image)
                            ...
                            <label class="flex flex-col items-center justify-center w-full h-24 border-2 border-dashed border-slate-200 rounded-lg cursor-pointer hover:border-primary hover:bg-slate-50 transition-colors bg-slate-50/50">
                                <div class="flex flex-col items-center justify-center pt-4 pb-4">
                                    ...
                                    <p class="text-[13px] text-body">Seleccionar imagen</p>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.general.banner_image_label') }}</label>
                        <div class="relative">
                            @if($banner_image)
                            <div class="mb-3 relative">
