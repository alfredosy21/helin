<div class="min-h-screen pb-12 bg-[#f8fafc] relative">
    ...
    <div class="relative z-10 p-6 space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-2">
            <div>
                <x-cms-breadcrumb :module-id="6" :submodule-id="17" />
                <p class="text-sm text-slate-500 mt-2.5">
                    {{ __('cms.resource_specialties.breadcrumb') }}
                </p>
            </div>
            ...
            <button wire:click="create" class="rounded-lg bg-primary hover:bg-[#079d8b] text-white px-4 py-2.5 text-sm font-medium transition-colors inline-flex items-center shadow-none border-none cursor-pointer">
                ...
                {{ __('cms.resource_specialties.new_button') }}
            </button>
            @endif
        </div>
        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-[0_1px_2px_0_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="p-4 bg-white border-b border-slate-50 flex flex-col md:flex-row gap-3">
                <div class="relative flex-1">
                    ...
                    <input type="text" wire:model.live="search" placeholder="{{ __('cms.resource_specialties.search_placeholder') }}"
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody id="resource-specialties-table-body" class="divide-y divide-slate-50 text-sm">
                        @forelse($resourceSpecialties as $resourceSpecialty)
                        <tr wire:key="resource-specialty-{{ $resourceSpecialty->id }}" data-id="{{ $resourceSpecialty->id }}" class="sortable-row hover:bg-slate-50/50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="flex items-center gap-2">
                                        @if($resourceSpecialty->icon)
                                        <div class="w-8 h-8 rounded-lg flex items-center justify-center" style="background-color: {{ $resourceSpecialty->color ?? '#f3f4f6' }};">
                                            <i class="{{ $resourceSpecialty->icon }} text-white text-sm"></i>
                                        </div>
                                        @endif
                                        <div>
                                            <div class="font-medium text-[#222]">{{ $resourceSpecialty->name }}</div>
                                            <div class="text-xs text-slate-500">{{ $resourceSpecialty->slug }}</div>
                                        </div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="text-xs text-slate-500">
                                    {{ $resourceSpecialty->updated_at->format('d/m/Y H:i') }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center gap-2">
                                    <span class="w-2 h-2 rounded-full {{ $resourceSpecialty->is_active ? 'bg-primary' : 'bg-slate-300' }}"></span>
                                    <span class="text-xs font-medium {{ $resourceSpecialty->is_active ? 'text-slate-700' : 'text-slate-400' }}">
                                        {{ $resourceSpecialty->is_active ? __('cms.general.status_active') : __('cms.general.status_inactive') }}
                                    </span>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end gap-1">
                                    <x-cms-tooltip text="{{ __('cms.general.edit') }}">
                                        <button wire:click="edit({{ $resourceSpecialty->id }})" class="p-2 text-slate-400 hover:text-primary hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                    ...
                                    <x-cms-tooltip text="{{ __('cms.general.delete') }}">
                                        <button onclick="deleteResourceSpecialty({{ $resourceSpecialty->id }})" class="p-2 text-slate-400 hover:text-red-500 hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center text-[#c0c1c6]">
                                    ...
                                    <p class="text-xs font-medium">{{ __('cms.resource_specialties.no_specialties') }}</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
        ...
        <div class="max-w-4xl mx-auto bg-white rounded-xl border border-slate-100 shadow-[0_1px_3px_0_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="p-6 border-b border-slate-50">
                <h2 class="text-lg font-bold text-[#222]">
                    {{ $editingId ? __('cms.resource_specialties.edit_title') : __('cms.resource_specialties.new_title') }}
                </h2>
                <p class="text-xs text-[#c0c1c6] mt-1">{{ __('cms.resource_specialties.subtitle') }}</p>
            </div>
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-1.5">
                            <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.resource_specialties.name_label') }} <span class="text-red-500">*</span></label>
                            <input type="text" wire:model="name" placeholder="{{ __('cms.resource_specialties.name_placeholder') }}"
                        ...
                        <div class="space-y-1.5">
                            <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.resource_specialties.slug_label') }} <span class="text-red-500">*</span></label>
                            <input type="text" wire:model="slug" placeholder="{{ __('cms.resource_specialties.slug_placeholder') }}"
                                   class="w-full px-3 py-2.5 bg-slate-50 border border-slate-100 text-sm text-slate-700 rounded-lg focus:outline-none focus:border-primary transition-colors placeholder-slate-300" />
                            @error('slug') <span class="text-xs text-red-500 font-medium italic">{{ $message }}</span> @enderror
                            <p class="text-xs text-[#c0c1c6] italic mt-1">{{ __('cms.resource_specialties.slug_helper') }}</p>
                        </div>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-1.5">
                            <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.resource_specialties.icon_label') }}</label>
                            <input type="text" wire:model="icon" placeholder="{{ __('cms.resource_specialties.icon_placeholder') }}"
                                   class="w-full px-3 py-2.5 bg-slate-50 border border-slate-100 text-sm text-slate-700 rounded-lg focus:outline-none focus:border-primary transition-colors placeholder-slate-300" />
                            @error('icon') <span class="text-xs text-red-500 font-medium italic">{{ $message }}</span> @enderror
                        </div>
                        <div class="space-y-1.5">
                            <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.resource_specialties.color_label') }}</label>
                            <input type="text" wire:model="color" placeholder="{{ __('cms.resource_specialties.color_placeholder') }}"
                                   class="w-full px-3 py-2.5 bg-slate-50 border border-slate-100 text-sm text-slate-700 rounded-lg focus:outline-none focus:border-primary transition-colors placeholder-slate-300" />
                    ...
                    {{-- Descripción --}}
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.resource_specialties.description_label') }}</label>
                        <textarea wire:model="description" rows="3"
                                  ...
                                  placeholder="{{ __('cms.resource_specialties.description_placeholder') }}"></textarea>
                        @error('description') <span class="text-xs text-red-500 font-medium italic">{{ $message }}</span> @enderror
                    </div>

                    {{-- Posición --}}
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.resource_specialties.position_label') }}</label>
                        <input type="number" wire:model="position" min="0" placeholder="{{ __('cms.resource_specialties.position_placeholder') }}"
                               class="w-full px-3 py-2.5 bg-slate-50 border border-slate-100 text-sm text-slate-700 rounded-lg focus:outline-none focus:border-primary transition-colors placeholder-slate-300" />
                    ...
                    {{-- Configuración JSON --}}
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.resource_specialties.config_label') }}</label>
                        <textarea wire:model="config" rows="4"
                                  ...
                                  placeholder="{{ __('cms.resource_specialties.config_placeholder') }}"></textarea>
                ...
                <div class="p-6 border-t border-slate-50 bg-slate-50/30 flex justify-end gap-3">
                    ...
                    <button type="submit" wire:loading.attr="disabled" wire:loading.class="opacity-75 cursor-not-allowed" class="px-6 py-2.5 rounded-lg text-sm font-medium bg-primary hover:bg-[#079d8b] text-white transition-colors border-none cursor-pointer flex items-center justify-center gap-2">
                        ...
                        <span wire:loading.remove wire:target="save">
                            {{ $editingId ? __('cms.general.save') : __('cms.resource_specialties.new_button') }}
                        </span>
                        <span wire:loading wire:target="save">
                            {{ $editingId ? __('cms.general.save') : __('cms.resource_specialties.new_button') }}
                        </span>
    ...
    <script>
        function deleteResourceSpecialty(resourceSpecialtyId) {
        window.confirmDelete({
        title: '{{ __('cms.resource_specialties.delete_title') }}',
                text: '{{ __('cms.general.delete_confirm_text_with_associated') }}',
                confirmButtonText: '{{ __('cms.resource_specialties.delete_button') }}',
                cancelButtonText: '{{ __('cms.general.cancel') }}',
                onConfirm: function() {
                Livewire.find('{{ $this->getId() }}').confirmDelete(resourceSpecialtyId);
                }
        });
        }
        ...
        const tbody = document.getElementById('resource-specialties-table-body');
        if (!tbody) return;
