<div class="min-h-screen pb-12 bg-soft relative">
    ...
    <div class="relative z-10 p-6 space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-2">
            <div>
                <x-cms-breadcrumb :module-id="\App\Models\Module::SETTINGS" :submodule-id="\App\Models\Submodule::PAGE_SEO" />
                <p class="text-sm text-slate-500 mt-2.5">
                    {{ __('cms.page_seo.breadcrumb') }}
                </p>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/70 border-b border-slate-100 text-slate-400 text-xs font-semibold">
                            <th class="px-6 py-3.5">{{ __('cms.page_seo.page_slug') }}</th>
                            <th class="px-6 py-3.5">{{ __('cms.page_seo.seo_title') }}</th>
                    ...
                    <tbody class="divide-y divide-slate-50 text-sm">
                        ...
                        <tr wire:key="page-seo-{{ $pageSeo->id }}" class="hover:bg-slate-50/50 transition-colors">
                            <td class="px-6 py-4">
                                <span class="font-semibold text-heading">{{ $pageSeo->page_slug }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-sm text-slate-600">{{ $pageSeo->seo_title ?? '—' }}</span>
                            </td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end gap-1">
                                    <x-cms-tooltip text="{{ __('cms.general.edit') }}">
                                        <button wire:click="edit({{ $pageSeo->id }})" class="p-2 text-slate-400 hover:text-primary hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                    ...
                                    <x-cms-tooltip text="{{ __('cms.general.delete') }}">
                                        <button onclick="deletePageSeo({{ $pageSeo->id }})" class="p-2 text-slate-400 hover:text-red-500 hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center text-slate-400">
                                    ...
                                    <p class="text-xs font-medium">{{ __('cms.page_seo.no_records') }}</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
        ...
        <div class="max-w-4xl mx-auto bg-white rounded-xl border border-slate-100 shadow-[0_1px_3px_0_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="p-6 border-b border-slate-50">
                ...
                <p class="text-xs text-slate-400 mt-1">{{ __('cms.page_seo.subtitle') }}</p>
            </div>
            ...
            <form wire:submit.prevent="save" class="w-full">
                <div class="p-6 space-y-6">
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ __('cms.page_seo.page_slug') }} <span class="text-red-500">*</span></label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ __('cms.page_seo.seo_title') }}</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ __('cms.page_seo.seo_keywords') }}</label>
                    ...
                    {{-- Imagen OG --}}
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ __('cms.page_seo.og_image') }}</label>
                        <div class="relative">
                            @if($og_image)
                            <div class="mb-3 relative">
                                <img src="{{ $og_image->temporaryUrl() }}" class="w-full h-32 object-cover rounded-lg border border-slate-100">
                                <button type="button" wire:click="$set('og_image', null)" class="absolute top-2 right-2 p-1 bg-white rounded-lg shadow-sm text-red-500 hover:text-red-700 border-none cursor-pointer">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                                </button>
                            </div>
                            @elseif($current_og_image)
                            <div class="mb-3 relative">
                                <img src="{{ asset('storage/' . $current_og_image) }}" class="w-full h-32 object-cover rounded-lg border border-slate-100">
                                <button type="button" wire:click="$set('current_og_image', null)" class="absolute top-2 right-2 p-1 bg-white rounded-lg shadow-sm text-red-500 hover:text-red-700 border-none cursor-pointer">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                                </button>
                            </div>
                            @endif
                            <label class="flex flex-col items-center justify-center w-full h-24 border-2 border-dashed border-slate-200 rounded-lg cursor-pointer hover:border-primary hover:bg-slate-50 transition-colors bg-slate-50/50">
                                <div class="flex flex-col items-center justify-center pt-4 pb-4">
                                    ...
                                    <p class="text-xs text-slate-500">{{ __('cms.page_seo.select_image') }}</p>
                                    <p class="text-[10px] text-slate-400 mt-0.5">JPG, PNG (Máx. 2MB)</p>
                                </div>
                                <input type="file" wire:model="og_image" class="hidden" accept="image/*" />
                            </label>
                            @error('og_image') <span class="text-xs text-red-500 font-medium italic">{{ $message }}</span> @enderror
                        </div>
                    </div>
