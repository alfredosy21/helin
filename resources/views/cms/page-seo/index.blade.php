<div class="min-h-screen pb-12 bg-soft relative">
    ...
    <div class="relative z-10 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-6">
        ...
        <x-ui-section-header :module-id="\App\Models\Module::SETTINGS" :submodule-id="\App\Models\Submodule::PAGE_SEO" :subtitle="__('cms.page_seo.title')">
            @if(!$showForm)
            <x-slot:action>
                    ...
                    <tbody class="divide-y divide-slate-100 text-[13px]">
                        ...
                        <tr wire:key="page-seo-{{ $pageSeo->id }}" class="hover:bg-slate-50/70 transition-colors duration-150">
                            <td class="px-4 py-2.5">
                                <span class="font-semibold text-heading block truncate">{{ $pageSeo->page_slug }}</span>
                            </td>
                            <td class="px-4 py-2.5">
                                <span class="text-[13px] text-body block truncate">{{ $pageSeo->seo_title ?? '—' }}</span>
                            </td>
        ...
        <form wire:submit.prevent="save" class="space-y-4 sm:space-y-5">
            ...
            {{-- Información básica --}}
            <x-ui-form-card title="{{ $editingId ? __('cms.page_seo.edit_title') : __('cms.page_seo.new_title') }}" description="{{ __('cms.page_seo.subtitle') }}" icon="search">
                <div class="space-y-4">
