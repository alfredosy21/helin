<div class="min-h-screen pb-12 bg-soft relative">
    ...
    <div class="relative z-10 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-6">
        ...
        <x-ui-section-header :module-id="\App\Models\Module::SETTINGS" :submodule-id="\App\Models\Submodule::DELIVERY_METHODS" :subtitle="__('cms.delivery_methods.title')">
            @if(!$showForm)
            <x-slot:action>
        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-[0_1px_2px_0_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody id="delivery-methods-table-body" class="divide-y divide-slate-100 text-[13px]">
                        @forelse($deliveryMethods as $method)
                        <tr wire:key="delivery-method-{{ $method->id }}" data-id="{{ $method->id }}" class="sortable-row hover:bg-slate-50/70 transition-colors duration-150">
                            ...
                            <td class="px-4 py-2.5 text-right">
                                <div class="flex justify-end gap-1">
                                    ...
                                    <x-cms-tooltip text="{{ __('cms.general.delete') }}">
                                        <button onclick="deleteDeliveryMethod({{ $method->id }})" class="p-2 text-body hover:text-red-500 hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                        ...
                        <tr>
                            <td colspan="5" class="px-4 py-8 text-center">
                                <x-ui-empty-state icon="folder" :title="__('cms.delivery_methods.empty')" />
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
            ...
            <form wire:submit.prevent="save" class="w-full">
                <div class="p-4 sm:p-6 space-y-4 sm:space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        ...
                        <div class="space-y-1.5">
                            <label class="text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.delivery_methods.slug_label') }}</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.delivery_methods.description_label') }}</label>
