    ...
    <div class="relative z-10 p-6 space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-2">
            ...
            <button wire:click="create" class="rounded-lg bg-primary hover:bg-[#079d8b] text-white px-4 py-2.5 text-sm font-medium transition-colors inline-flex items-center shadow-none border-none cursor-pointer">
                ...
                {{ __('cms.delivery_methods.new_button') }}
            </button>
            @endif
        </div>
        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-[0_1px_2px_0_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="p-4 bg-white border-b border-slate-50 flex flex-col md:flex-row gap-3">
                <div class="relative flex-1">
                    ...
                    <input type="text" wire:model.live="search" placeholder="{{ __('cms.delivery_methods.search_placeholder') }}"
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody id="delivery-methods-table-body" class="divide-y divide-slate-50 text-sm">
                        @forelse($deliveryMethods as $method)
                        <tr wire:key="delivery-method-{{ $method->id }}" data-id="{{ $method->id }}" class="sortable-row hover:bg-slate-50/50 transition-colors">
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-center">
                                <span class="text-xs text-slate-500">{{ $method->updated_at->format('d/m/Y H:i') }}</span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <x-ui-badge-status :active="$method->is_active" :active-label="__('cms.general.status_active')" :inactive-label="__('cms.general.status_inactive')" />
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end gap-1">
                                    ...
                                    <x-cms-tooltip text="{{ __('cms.general.delete') }}">
                                        <button onclick="deleteDeliveryMethod({{ $method->id }})" class="p-2 text-slate-400 hover:text-red-500 hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center text-slate-400">
                                    <svg class="w-10 h-10 mb-2 stroke-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 18.75a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 0 1-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 0 0-3.213-9.193 2.056 2.056 0 0 0-1.58-.86H14.25M16.5 18.75h-2.25m0-11.177v-.958c0-.568-.422-1.048-.987-1.106a48.554 48.554 0 0 0-10.026 0 1.106 1.106 0 0 0-.987 1.106v7.635m12-6.677v6.677m0 4.5v-4.5m0 0h-12"/></svg>
                                    <p class="text-xs font-medium">{{ __('cms.delivery_methods.empty') }}</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
        ...
        <div class="max-w-4xl mx-auto bg-white rounded-xl border border-slate-100 shadow-[0_1px_3px_0_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="p-6 border-b border-slate-50">
                ...
                <p class="text-xs text-slate-400 mt-1">{{ __('cms.delivery_methods.subtitle') }}</p>
            </div>
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-1.5">
                            <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ __('cms.delivery_methods.name_label') }} <span class="text-red-500">*</span></label>
                        ...
                        <div class="space-y-1.5">
                            <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ __('cms.delivery_methods.slug_label') }}</label>
                            ...
                            <p class="text-xs text-slate-400 italic mt-1">{{ __('cms.delivery_methods.slug_helper') }}</p>
                        </div>
                    </div>

                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ __('cms.delivery_methods.description_label') }}</label>
    ...
    <script>
        function deleteDeliveryMethod(id) {
            window.confirmDelete({
                title: '{{ __('cms.delivery_methods.delete_title') }}',
                text: '{{ __('cms.general.delete_confirm_text_with_associated') }}',
                confirmButtonText: '{{ __('cms.delivery_methods.delete_button') }}',
