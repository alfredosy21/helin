<div class="min-h-screen pb-12 bg-soft relative">
    ...
    <div class="relative z-10 p-6 space-y-6">
        ...
        <x-ui-section-header :module-id="\App\Models\Module::CATALOG" :submodule-id="\App\Models\Submodule::PRODUCT_BRANDS" :subtitle="__('cms.brands.title')">
            @if(!$showForm)
            <x-slot:action>
        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-[0_1px_2px_0_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="p-4 bg-white border-b border-slate-50 flex flex-col md:flex-row gap-3">
                <div class="relative flex-1">
                    ...
                    <input type="text" wire:model.live="search" placeholder="{{ __('cms.brands.search_placeholder') }}"
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody id="brands-table-body" class="divide-y divide-slate-50 text-sm">
                        ...
                        <tr wire:key="brand-{{ $brand->id }}" data-id="{{ $brand->id }}" class="sortable-row hover:bg-slate-50/50 transition-colors">
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-center">
                                <x-ui-badge-status :active="$brand->is_active" :active-label="__('cms.general.status_active')" :inactive-label="__('cms.general.status_inactive')" />
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end gap-1">
                                    <x-cms-tooltip text="{{ __('cms.general.edit') }}">
                                        <button wire:click="edit({{ $brand->id }})" class="p-2 text-slate-400 hover:text-primary hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                    ...
                                    <x-cms-tooltip text="{{ __('cms.general.delete') }}">
                                        <button onclick="deleteBrand({{ $brand->id }})" class="p-2 text-slate-400 hover:text-red-500 hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        <tr>
                            <td colspan="4">
                                <x-ui-empty-state icon="folder" title="No se encontraron marcas" />
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="max-w-4xl mx-auto bg-white rounded-xl border border-slate-100 shadow-[0_1px_3px_0_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="p-6 border-b border-slate-50">
                ...
                <p class="text-xs text-slate-400 mt-1">{{ __('cms.brands.subtitle') }}</p>
            </div>
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-1 gap-6">
                        <div class="space-y-1.5">
                            <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ __('cms.brands.name_label') }} <span class="text-red-500">*</span></label>
    ...
    <script>
        ...
        title: '{{ __('cms.brands.delete_title') }}',
                text: '{{ __('cms.general.delete_confirm_text_with_associated') }}',
                confirmButtonText: '{{ __('cms.brands.delete_button') }}',
                cancelButtonText: '{{ __('cms.general.cancel') }}',
                onConfirm: function() {
                Livewire.find('{{ $this->getId() }}').confirmDelete(brandId);
                }
        });
        }
