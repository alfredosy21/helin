<div class="min-h-screen pb-12 bg-[#f8fafc] relative">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-2">
            <div>
                <div class="flex items-center gap-2 text-xs text-slate-400 mb-1 font-medium tracking-wide uppercase">
                    ...
                    <span class="text-primary-600 font-semibold">{{ __('cms.brands.breadcrumb') }}</span>
                </div>
                            ...
                            <tr wire:key="brand-{{ $brand->id }}" data-id="{{ $brand->id }}" class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-1">
                                        <button wire:click="edit({{ $brand->id }})" class="p-2 text-slate-400 hover:text-primary hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer" title="{{ __('cms.general.edit') }}">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125"/>
                                            </svg>
                                        </button>
                                        <button onclick="openDeleteModal({{ $brand->id }})" class="p-2 text-slate-400 hover:text-red-500 hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer" title="{{ __('cms.general.delete') }}">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <div class="relative w-full max-w-md h-full bg-white shadow-xl flex flex-col border-l border-slate-100">
            <div class="p-6 border-b border-slate-50 flex justify-between items-center">
                <div>
                    ...
                    <p class="text-xs text-[#c0c1c6]">{{ __('cms.brands.subtitle') }}</p>
                </div>
            ...
            <form wire:submit.prevent="save" class="flex flex-col flex-1 h-full">
                <div class="flex-1 overflow-y-auto p-6 space-y-5">
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.brands.name_label') }}</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.brands.logo_label') }}</label>
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        ...
        <div class="fixed inset-0 flex items-center justify-center p-4">
            <div class="relative bg-white rounded-xl shadow-xl w-full max-w-sm border border-slate-100 transform transition-all p-6 text-center">
                ...
                <h3 class="text-base font-bold text-[#222] mb-1">{{ __('cms.brands.delete_title') }}</h3>
    ...
    <script>
        ...
        function confirmDelete() {
            if (deleteBrandId) {
                Livewire.find('{{ $this->getId() }}').confirmDelete(deleteBrandId);
                closeDeleteModal();
            }
        }
    </script>
