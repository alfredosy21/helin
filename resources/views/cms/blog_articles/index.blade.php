    ...
    <div class="relative z-10 p-6 space-y-6">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/60 text-[10px] font-semibold text-body uppercase tracking-wider border-b border-slate-100">
                            <th class="px-4 py-2.5">{{ __('cms.tables.article') }}</th>
                            <th class="px-4 py-2.5">{{ __('cms.tables.author') }}</th>
                    ...
                    <tbody id="articles-table-body" class="divide-y divide-slate-100 text-[13px]">
                        @forelse($articles as $article)
                        <tr wire:key="article-{{ $article->id }}" data-id="{{ $article->id }}" class="hover:bg-slate-50/70 transition-colors duration-150">
                            ...
                            <td class="px-4 py-2.5 text-body text-xs">
                                {{ $article->author ?? __('cms.blog_articles.no_author') }}
                            </td>
                            <td class="px-4 py-2.5">
                                @if($article->blogCategory)
                                <span class="text-[13px] text-body">{{ $article->blogCategory->name }}</span>
                                @else
                                <span class="text-[13px] text-body italic">{{ __('cms.blog_articles.no_category') }}</span>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-2.5 text-center">
                                <button wire:click="toggleStatus({{ $article->id }})"
                                        class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-semibold transition-colors cursor-pointer border-none {{ $article->is_active ? 'bg-primary-500/10 text-primary-700' : 'bg-slate-100 text-body' }}">
                                    <span class="w-1.5 h-1.5 rounded-full {{ $article->is_active ? 'bg-primary-500' : 'bg-slate-400' }}"></span>
                                ...
                                <div class="flex justify-end gap-1">
                                    <x-cms-tooltip text="{{ $article->is_featured ? __('cms.blog_articles.remove_featured') : __('cms.blog_articles.mark_featured') }}">
                                        <button wire:click="toggleFeatured({{ $article->id }})"
                                                class="p-2 {{ $article->is_featured ? 'text-yellow-500 bg-yellow-50' : 'text-body hover:text-yellow-500 hover:bg-yellow-50' }} rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    ...
                                    <x-cms-tooltip text="{{ __('cms.general.edit') }}">
                                        <button wire:click="edit({{ $article->id }})" class="p-2 text-body hover:text-primary hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    ...
                                    <x-cms-tooltip text="{{ __('cms.general.delete') }}">
                                        <button onclick="openDeleteModal({{ $article->id }})" class="p-2 text-body hover:text-red-500 hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        @empty
                        <tr>
                            <td colspan="6" class="px-4 py-8 text-center"><x-ui-empty-state icon="folder" :title="__('cms.general.no_records')" />
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="max-w-4xl mx-auto bg-white rounded-xl border border-slate-100 shadow-[0_1px_3px_0_rgba(0,0,0,0.02)] overflow-hidden animate-in fade-in duration-200">
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-1.5">
                        ...
                        <input type="text" wire:model="title" required placeholder="{{ __('cms.blog_articles.title_placeholder') }}"
                               class="w-full rounded-lg border border-slate-100 bg-slate-50 px-2.5 py-1.5 text-[13px] text-body focus:border-primary focus:outline-none transition-colors placeholder-body" />
                        @error('title') <p class="mt-1 text-xs text-red-500 font-medium italic">{{ $message }}</p> @enderror
                    </div>
                    <div class="space-y-1.5">
                        <label class="block text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.blog_articles.author_label') }}</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-1.5">
                        ...
                        <input type="text" wire:model="slug" placeholder="{{ __('cms.blog_articles.slug_placeholder') }}"
                               class="w-full rounded-lg border border-slate-100 bg-slate-50 px-2.5 py-1.5 text-[13px] text-body focus:border-primary focus:outline-none transition-colors placeholder-body" />
                        <p class="mt-1 text-[10px] text-body">{{ __('cms.categories.slug_helper') }}</p>
                        @error('slug') <p class="mt-1 text-xs text-red-500 font-medium italic">{{ $message }}</p> @enderror
                    </div>
                    <div class="space-y-1.5">
                        <label class="block text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.blog_articles.category_label') }}</label>
                        <select wire:model="blog_category_id"
                                class="w-full rounded-lg border border-slate-100 bg-slate-50 px-2.5 py-1.5 text-[13px] text-body focus:border-primary focus:outline-none transition-colors">
                ...
                {{-- Extracto --}}
                <div class="space-y-1.5">
                    <label class="block text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.blog_articles.excerpt_label') }}</label>
                    <textarea wire:model="excerpt" rows="2" placeholder="{{ __('cms.blog_articles.excerpt_placeholder') }}"
                              class="w-full rounded-lg border border-slate-100 bg-slate-50 px-2.5 py-1.5 text-[13px] text-body focus:border-primary focus:outline-none transition-colors resize-none placeholder-body"></textarea>
                    @error('excerpt') <p class="mt-1 text-xs text-red-500 font-medium italic">{{ $message }}</p> @enderror
                </div>

                {{-- Quill Editor Ampliado --}}
                <div class="space-y-1.5">
                    <label class="block text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.blog_articles.content_label') }} <span class="text-red-500">*</span></label>
                ...
                <div class="border-t border-slate-100 pt-5">
                    <h4 class="text-sm font-bold text-body mb-4">{{ __('cms.blog_articles.seo_section') }}</h4>
                    <div class="space-y-4">
                        <div class="space-y-1.5">
                            <label class="block text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.blog_articles.meta_title_label') }}</label>
                        ...
                        <div class="space-y-1.5">
                            <label class="block text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.blog_articles.meta_description_label') }}</label>
                        ...
                        <div class="space-y-1.5">
                            <label class="block text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.blog_articles.meta_keywords_label') }}</label>
