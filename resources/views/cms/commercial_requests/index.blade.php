    ...
    <div class="relative z-10 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-6">
        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-[0_1px_2px_0_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="p-3 sm:p-4 bg-white border-b border-slate-50 flex flex-col sm:flex-row gap-2 sm:gap-3">
                ...
                <select wire:model.live="statusFilter" class="cms-perpage-select bg-slate-50 border border-slate-100 rounded-lg px-3 py-1.5 pr-8 text-[13px] text-body focus:outline-none focus:border-primary transition-colors appearance-none cursor-pointer">
                        ...
                        <tr wire:key="request-{{ $request->id }}" class="hover:bg-slate-50/70 transition-colors duration-150">
                            ...
                            <td class="px-4 py-2.5 text-center">
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium
                                    ...
                                    @elseif($request->status === 'completed') bg-primary-500/10 text-primary-700
                            ...
                            <td class="px-4 py-2.5 text-center">
                                <span class="text-[13px] text-body">{{ $request->created_at->format('d/m/Y H:i') }}</span>
                            </td>
                            <td class="px-4 py-2.5 text-center">
                                <div class="flex justify-center gap-1">
                                    @if($request->uuid)
                                    <x-cms-tooltip text="Ver página de solicitud">
                                        <a href="{{ route('solicitud-enviada', ['uuid' => $request->uuid]) }}" target="_blank" class="p-2 text-body hover:text-primary hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                    ...
                                    <x-cms-tooltip text="Ver detalles">
                                        <button type="button" wire:click="viewDetails({{ $request->id }})" class="p-2 text-body hover:text-primary hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    ...
                                    <x-cms-tooltip text="Eliminar">
                                        <button onclick="deleteRequest({{ $request->id }})" class="p-2 text-body hover:text-red-500 hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" wire:click="closeDetails">
        <div class="bg-white rounded-xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto" wire:click.stop>
            <div class="p-6 border-b border-slate-50 flex justify-between items-center">
                ...
                <button wire:click="closeDetails" class="text-body hover:text-body border-none bg-transparent cursor-pointer">
            ...
            <div class="p-4 sm:p-6 space-y-4 sm:space-y-6">
                ...
                <div class="bg-slate-50 rounded-lg p-4">
                    ...
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            <span class="text-body">Nombre:</span>
                            <span class="text-body ml-2">{{ $selectedRequest->full_name }}</span>
                        </div>
                        <div>
                            <span class="text-body">Tipo:</span>
                            <span class="text-body ml-2">{{ $selectedRequest->customerType->name ?? 'N/A' }}</span>
                        </div>
                        @if($selectedRequest->cedula)
                        <div>
                            <span class="text-body">Cédula:</span>
                            <span class="text-body ml-2">{{ $selectedRequest->cedula }}</span>
                        </div>
                        @endif
                        @if($selectedRequest->company_name)
                        <div>
                            <span class="text-body">Empresa:</span>
                            <span class="text-body ml-2">{{ $selectedRequest->company_name }}</span>
                        </div>
                        @endif
                        @if($selectedRequest->rif)
                        <div>
                            <span class="text-body">RIF:</span>
                            <span class="text-body ml-2">{{ $selectedRequest->rif }}</span>
                        </div>
                        @endif
                        <div>
                            <span class="text-body">Teléfono:</span>
                            <span class="text-body ml-2">{{ $selectedRequest->phone }}</span>
                        </div>
                        <div>
                            <span class="text-body">Email:</span>
                            <span class="text-body ml-2">{{ $selectedRequest->email }}</span>
                        </div>
                    ...
                    <div class="mt-3">
                        <span class="text-body text-sm">Dirección:</span>
                        <p class="text-body text-sm mt-1">{{ $selectedRequest->address }}</p>
                    </div>
                    <div class="mt-3 grid grid-cols-2 gap-4 text-sm">
                        <div>
                            <span class="text-body">Estado:</span>
                            <span class="text-body ml-2">{{ $selectedRequest->state->name ?? 'N/A' }}</span>
                        </div>
                        <div>
                            <span class="text-body">Ciudad:</span>
                            <span class="text-body ml-2">{{ $selectedRequest->city->name ?? 'N/A' }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-slate-50 rounded-lg p-4">
                    ...
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            <span class="text-body">Método:</span>
                            <span class="text-body ml-2">{{ $selectedRequest->deliveryMethod->name ?? 'N/A' }}</span>
                        </div>
                        @if($selectedRequest->other_delivery_company)
                        <div>
                            <span class="text-body">Otra Empresa:</span>
                            <span class="text-body ml-2">{{ $selectedRequest->other_delivery_company }}</span>
                        </div>
                        @endif
                    </div>
                    ...
                    <div class="mt-3 grid grid-cols-2 gap-4 text-sm">
                        <div>
                            <span class="text-body">Destinatario:</span>
                            <span class="text-body ml-2">{{ $selectedRequest->recipient_name }}</span>
                        </div>
                        <div>
                            <span class="text-body">Documento:</span>
                            <span class="text-body ml-2">{{ $selectedRequest->recipient_document }}</span>
                        </div>
                        <div>
                            <span class="text-body">Teléfono:</span>
                            <span class="text-body ml-2">{{ $selectedRequest->recipient_phone }}</span>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="mt-3 grid grid-cols-2 gap-4 text-sm">
                        <div>
                            <span class="text-body">Estado Envío:</span>
                            <span class="text-body ml-2">{{ $selectedRequest->shippingState->name ?? 'N/A' }}</span>
                        </div>
                        <div>
                            <span class="text-body">Ciudad Envío:</span>
                            <span class="text-body ml-2">{{ $selectedRequest->shippingCity->name ?? 'N/A' }}</span>
                        </div>
                        <div class="col-span-2">
                            <span class="text-body">Agencia Destino:</span>
                            <span class="text-body ml-2">{{ $selectedRequest->destination_agency }}</span>
                        </div>
                    </div>
                    @endif
                </div>
                ...
                <div class="bg-slate-50 rounded-lg p-4">
                    ...
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            <span class="text-body">Método:</span>
                            <span class="text-body ml-2">{{ $selectedRequest->paymentMethod->name ?? 'N/A' }}</span>
                        </div>
                        @if($selectedRequest->payment_receipt_number)
                        <div>
                            <span class="text-body">Comprobante:</span>
                            <span class="text-body ml-2">{{ $selectedRequest->payment_receipt_number }}</span>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="bg-slate-50 rounded-lg p-4">
                    ...
                    <p class="text-body text-sm">{{ $selectedRequest->observations }}</p>
                </div>
                @endif
                    ...
                    <div class="text-sm">
                        @if($selectedRequest->cart_data)
                            ...
                            <div class="flex justify-between py-2 border-b border-slate-200 last:border-0">
                                <span class="text-body">{{ $item['name'] ?? 'Producto' }} x{{ $item['quantity'] ?? 1 }}</span>
                                <span class="text-body">{{ $item['price'] ?? 0 }}</span>
                            </div>
                            @endforeach
                        @else
                        <p class="text-body">No hay información del carrito</p>
                        @endif
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-slate-200">
                    <div class="flex gap-2">
                        <button wire:click="updateStatus({{ $selectedRequest->id }}, 'pending')" class="px-4 py-2 rounded-lg text-sm font-medium bg-amber-50 text-amber-600 hover:bg-amber-100 transition-colors border-none cursor-pointer">
                            Pendiente
                        </button>
                        ...
                        <button wire:click="updateStatus({{ $selectedRequest->id }}, 'completed')" class="px-4 py-2 rounded-lg text-sm font-medium bg-primary-500/10 text-primary-700 hover:bg-primary-500/20 transition-colors border-none cursor-pointer">
                            Completada
                        </button>
