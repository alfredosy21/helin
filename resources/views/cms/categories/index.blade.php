        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-[0_1px_2px_0_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody id="categories-table-body" class="divide-y divide-slate-50 text-sm">
                        @forelse($categories as $category)
                        <tr wire:key="category-{{ $category->id }}" data-id="{{ $category->id }}" class="sortable-row hover:bg-slate-50/50 transition-colors">
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end gap-1">
                                    <x-cms-tooltip text="{{ __('cms.general.edit') }}">
                                        <button wire:click="edit({{ $category->id }})" class="p-2 text-slate-400 hover:text-primary hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                    ...
                                    <x-cms-tooltip text="{{ __('cms.general.delete') }}">
                                        <button onclick="deleteCategory({{ $category->id }})" class="p-2 text-slate-400 hover:text-red-500 hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        <tr>
                            <td colspan="4">
                                <x-ui-empty-state icon="folder" title="No se encontraron categorías" />
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="max-w-4xl mx-auto bg-white rounded-xl border border-slate-100 shadow-[0_1px_3px_0_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="p-6 border-b border-slate-50">
                <h2 class="text-lg font-bold text-heading">
                    {{ $editingId ? __('cms.categories.edit_title') : __('cms.categories.new_title') }}
                </h2>
                <p class="text-xs text-slate-400 mt-1">{{ __('cms.categories.subtitle') }}</p>
            </div>
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-1.5">
                            <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ __('cms.categories.name_label') }} <span class="text-red-500">*</span></label>
                        ...
                        <div class="space-y-1.5">
                            <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ __('cms.categories.slug_label') }}</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ __('cms.categories.description_label') }}</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ __('cms.categories.seo_description_label') }}</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">SEO Keywords</label>
                    ...
                    {{-- Banner --}}
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ __('cms.general.banner_label_label') }}</label>
