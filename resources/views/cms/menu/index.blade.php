...
<div class="min-h-screen pb-12 bg-soft relative">
    ...
    <div class="relative z-10 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-6">
        ...
        <x-ui-section-header :module-id="\App\Models\Module::SETTINGS" :submodule-id="\App\Models\Submodule::WEBSITE_MENU" :subtitle="__('cms.menu.title')">
            @if(!$showForm)
            <x-slot:action>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/60 text-[10px] font-semibold text-body uppercase tracking-wider border-b border-slate-100">
                            ...
                            <th class="px-4 py-2.5 text-center w-20">Tipo</th>
                            <th class="px-4 py-2.5 text-center w-24">Padre</th>
                            <th class="px-4 py-2.5 text-center w-24">Posición</th>
                            <th class="px-4 py-2.5 text-center w-24">Estado</th>
                            <th class="px-4 py-2.5 text-right w-40">Acciones</th>
                        </tr>
                    </thead>
                    <tbody id="menus-table-body" class="divide-y divide-slate-100 text-[13px]">
                        @forelse($menus as $menu)
                        <tr wire:key="menu-{{ $menu->id }}" data-id="{{ $menu->id }}" class="sortable-row hover:bg-slate-50/70 transition-colors duration-150">
                            ...
                            <td class="px-4 py-2.5 text-center">
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium
                                    @if($menu->type == 1) bg-primary-500/10 text-primary-700
                                    @elseif($menu->type == 2) bg-primary-500/10 text-primary-700
                                    @else bg-primary-500/10 text-primary-700 @endif">
                                    {{ $this->getTypeLabel($menu->type) }}
                                </span>
                            </td>
                            <td class="px-4 py-2.5 text-center">
                                <span class="text-[13px] text-body">{{ $this->getParentLabel($menu->parent_id) }}</span>
                            </td>
                            <td class="px-4 py-2.5 text-center">
                                <span class="text-xs font-medium text-body">{{ $menu->position }}</span>
                            </td>
                            <td class="px-4 py-2.5 text-center">
                            ...
                            <td class="px-4 py-2.5 text-right">
                                <div class="flex justify-end gap-1">
                                    <x-cms-tooltip text="Editar">
                                        <button wire:click="edit({{ $menu->id }})" class="p-2 text-body hover:text-primary hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                    ...
                                    <x-cms-tooltip text="Eliminar">
                                        <button onclick="deleteMenu({{ $menu->id }})" class="p-2 text-body hover:text-red-500 hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <form wire:submit.prevent="save" class="w-full">
                <div class="p-4 sm:p-6 space-y-4 sm:space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        ...
                        <div class="space-y-1.5">
                            <label class="text-[11px] font-semibold text-body uppercase tracking-wider">Tipo <span class="text-red-500">*</span></label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        ...
                        <div class="space-y-1.5">
                            <label class="text-[11px] font-semibold text-body uppercase tracking-wider">Posición</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        ...
                        <div class="space-y-1.5">
                            <label class="text-[11px] font-semibold text-body uppercase tracking-wider">Icono</label>
