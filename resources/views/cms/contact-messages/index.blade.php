                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/70 border-b border-slate-100 text-[#c0c1c6] text-xs font-semibold">
                            ...
                            <th class="px-4 py-3.5">Asunto</th>
                        ...
                        @forelse($messages as $message)
                            <tr class="hover:bg-slate-50/60 transition-colors {{ $message->is_read ? 'text-slate-500' : 'font-medium text-[#222]' }}">
                                ...
                                <td class="px-4 py-3.5">
                                    ...
                                    @if ($message->telefono)
                                        <div class="text-xs text-slate-400">{{ $message->telefono }}</div>
                                    @endif
                                </td>
                                <td class="px-4 py-3.5 max-w-[220px] truncate">{{ $message->asunto }}</td>
                                <td class="px-4 py-3.5 text-center">
                                    @if ($message->is_read)
                                        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs bg-slate-100 text-slate-500">
                                ...
                                <td class="px-4 py-3.5 text-center text-xs">{{ $message->created_at->format('d/m/Y H:i') }}</td>
                                <td class="px-4 py-3.5 text-center">
                                    <div class="flex items-center justify-center gap-1.5">
                                        ...
                                        <button wire:click="delete({{ $message->id }})" wire:confirm="¿Eliminar este mensaje?" title="Eliminar"
                                                class="p-2 rounded-lg hover:bg-red-50 text-slate-500 hover:text-red-500 transition-colors">
