<div class="min-h-screen pb-12 bg-soft relative">
    ...
    <div class="relative z-10 p-6 space-y-6">
        ...
        <x-ui-section-header :module-id="\App\Models\Module::CATALOG" :submodule-id="\App\Models\Submodule::ATTRIBUTE_VALUES" :subtitle="__('cms.attribute_values.breadcrumb')">
            @if(!$showForm)
            <x-slot:action>
                <button wire:click="create" class="rounded-lg bg-primary hover:bg-[#079d8b] text-white px-3 py-1.5 text-[13px] font-medium transition-colors inline-flex items-center shadow-none border-none cursor-pointer">
                    ...
                    {{ __('cms.attribute_values.new_button') }}
                </button>
            </x-slot:action>
            @endif
        </x-ui-section-header>
        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-[0_1px_2px_0_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="p-4 bg-white border-b border-slate-50 flex flex-col md:flex-row gap-3">
                ...
                <select wire:model.live="attributeFilter" class="cms-perpage-select bg-slate-50 border border-slate-100 rounded-lg px-3 py-1.5 pr-8 text-[13px] text-body focus:outline-none focus:border-primary transition-colors appearance-none cursor-pointer">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/60 text-[10px] font-semibold text-body uppercase tracking-wider border-b border-slate-100">
                            ...
                            <th class="px-4 py-2.5">{{ __('cms.attribute_values.value') }}</th>
                            <th class="px-4 py-2.5">{{ __('cms.attribute_values.label') }}</th>
                            <th class="px-4 py-2.5">{{ __('cms.attribute_values.color') }}</th>
                    ...
                    <tbody class="divide-y divide-slate-100 text-[13px]">
                            ...
                            <td class="px-4 py-2.5">
                                @if($attributeValue->color)
                                    <span class="inline-flex items-center gap-1.5 text-xs text-body">
                                        <span class="w-4 h-4 rounded-full border border-slate-200" style="background-color: {{ $attributeValue->color }}"></span>
                            ...
                            <td class="px-4 py-2.5 text-right">
                                <div class="flex justify-end gap-1">
                                    <x-cms-tooltip text="{{ __('cms.general.edit') }}">
                                        <button wire:click="edit({{ $attributeValue->id }})" class="p-2 text-body hover:text-primary hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                    ...
                                    <x-cms-tooltip text="{{ __('cms.general.delete') }}">
                                        <button onclick="deleteAttributeValue({{ $attributeValue->id }})" class="p-2 text-body hover:text-red-500 hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        <tr>
                            <td colspan="6" class="px-4 py-8 text-center">
                                <x-ui-empty-state icon="folder" :title="__('cms.attribute_values.no_values')" />
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
    ...
    <script>
        ...
        title: '{{ __('cms.attribute_values.delete_title') }}',
                text: '{{ __('cms.general.delete_confirm_text') }}',
                confirmButtonText: '{{ __('cms.attribute_values.delete_button') }}',
