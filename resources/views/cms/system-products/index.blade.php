<div class="min-h-screen pb-12 bg-soft relative">
    ...
    <div class="relative z-10 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-6">
                ...
                <table class="w-full table-fixed text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/60 text-[10px] font-semibold text-body uppercase tracking-wider border-b border-slate-100">
                            <th class="px-4 py-2.5">{{ __('cms.tables.system_product') }}</th>
                    ...
                    <tbody id="system-products-table-body" class="divide-y divide-slate-100 text-[13px]">
                        @forelse($systems as $system)
                        <tr wire:key="line-{{ $system->id }}" data-id="{{ $system->id }}" class="sortable-row hover:bg-slate-50/70 transition-colors duration-150">
                            ...
                            <td class="px-4 py-2.5 text-right">
                                <div class="flex justify-end gap-1">
                                    <x-cms-tooltip text="{{ __('cms.general.edit') }}">
                                        <button wire:click="edit({{ $system->id }})" class="p-2 text-body hover:text-primary hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                    ...
                                    <x-cms-tooltip text="{{ __('cms.general.delete') }}">
                                        <button onclick="deleteLine({{ $system->id }})" class="p-2 text-body hover:text-red-500 hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        @empty
                        <tr>
                            <td colspan="4" class="px-4 py-8 text-center"><x-ui-empty-state icon="folder" :title="__('cms.system_products.no_results')" />
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <form wire:submit.prevent="save" class="space-y-4 sm:space-y-5">
            ...
            {{-- Información básica --}}
            <x-ui-form-card title="{{ $editingId ? __('cms.system_products.edit_title') : __('cms.system_products.new_title') }}" description="{{ __('cms.system_products.subtitle') }}" icon="layers">
                <div class="space-y-4">
                    <x-ui-toggle wire:model="is_active" :label="__('cms.general.status_active')" />

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                        <div class="space-y-1.5">
                            <label class="text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.system_products.name_label') }} <span class="text-red-500">*</span></label>
                        ...
                        <div class="space-y-1.5">
                            <label class="text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.system_products.slug_label') }}</label>
            ...
            {{-- Banner --}}
            <x-ui-form-card title="Banner" description="Imagen promocional del sistema" icon="image">
                <div class="space-y-4">
                    <div class="space-y-1.5">
