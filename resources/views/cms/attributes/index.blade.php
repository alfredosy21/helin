<div class="min-h-screen pb-12 bg-[#f8fafc] relative">
    ...
    <div class="relative z-10 p-6 space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-2">
            <div>
                <x-cms-breadcrumb :module-id="3" :submodule-id="25" />
        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-[0_1px_2px_0_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="p-4 bg-white border-b border-slate-50 flex flex-col md:flex-row gap-3">
                <div class="relative flex-1">
                    ...
                    <input type="text" wire:model.live="search" placeholder="{{ __('cms.attributes.search_placeholder') }}"
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/70 border-b border-slate-100 text-[#c0c1c6] text-xs font-semibold">
                            <th class="px-6 py-3.5">{{ __('cms.attributes.name') }}</th>
                            <th class="px-6 py-3.5">{{ __('cms.attributes.type') }}</th>
                            <th class="px-6 py-3.5">{{ __('cms.attributes.filterable') }}</th>
                    ...
                    <tbody class="divide-y divide-slate-50 text-sm">
                        ...
                        <tr wire:key="attribute-{{ $attribute->id }}" class="hover:bg-slate-50/50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="font-medium text-[#222]">{{ $attribute->name }}</div>
                                @if($attribute->unit)
                                    <div class="text-xs text-slate-400">({{ $attribute->unit }})</div>
                                @endif
                                @if($attribute->description)
                                    <div class="text-xs text-slate-400 mt-0.5 max-w-md truncate">{{ $attribute->description }}</div>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-2.5 py-1 rounded-md bg-slate-50 border border-slate-100 text-xs text-slate-600">
                            ...
                            <td class="px-6 py-4">
                                @if($attribute->is_filterable)
                                    <span class="inline-flex items-center gap-1.5 text-xs font-medium text-primary">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="m3 4 7.5 8.6v5.1l3 2.3V12.6L21 4H3Z"/></svg>
                                        {{ __('cms.general.status_active') }}
                                    </span>
                                @else
                                    <span class="text-xs text-slate-400">{{ __('cms.general.status_inactive') }}</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-center">
                                <button wire:click="toggle({{ $attribute->id }})" class="inline-flex items-center gap-2 cursor-pointer border-none bg-transparent">
                                    <span class="w-2 h-2 rounded-full {{ $attribute->is_active ? 'bg-primary' : 'bg-slate-300' }}"></span>
                                    <span class="text-xs font-medium {{ $attribute->is_active ? 'text-slate-700' : 'text-slate-400' }}">
                                        {{ $attribute->is_active ? __('cms.general.status_active') : __('cms.general.status_inactive') }}
                                    </span>
                                </button>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end gap-1">
                                    <x-cms-tooltip text="{{ __('cms.general.edit') }}">
                                        <button wire:click="edit({{ $attribute->id }})" class="p-2 text-slate-400 hover:text-primary hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                    ...
                                    <x-cms-tooltip text="{{ __('cms.general.delete') }}">
                                        <button onclick="deleteAttribute({{ $attribute->id }})" class="p-2 text-slate-400 hover:text-red-500 hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <div class="max-w-4xl mx-auto bg-white rounded-xl border border-slate-100 shadow-[0_1px_3px_0_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="p-6 border-b border-slate-50">
                <h2 class="text-lg font-bold text-[#222]">
                    {{ $editingId ? __('cms.attributes.edit_title') : __('cms.attributes.new_title') }}
                </h2>
                <p class="text-xs text-[#c0c1c6] mt-1">{{ __('cms.attributes.subtitle') }}</p>
            </div>
            ...
            <form wire:submit.prevent="save" class="w-full">
                <div class="p-6 space-y-6">
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.attributes.name_label') }} <span class="text-red-500">*</span></label>
                        <input type="text" wire:model="name" placeholder="{{ __('cms.attributes.name_placeholder') }}"
                    ...
                    {{-- Slug --}}
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.attributes.slug_label') }}</label>
                        <input type="text" wire:model="slug" placeholder="{{ __('cms.attributes.slug_placeholder') }}"
                               class="w-full px-3 py-2.5 bg-slate-50 border border-slate-100 text-sm text-slate-700 rounded-lg focus:outline-none focus:border-primary transition-colors placeholder-slate-300" />
                        <p class="text-xs text-slate-400">{{ __('cms.attributes.slug_helper') }}</p>
                        @error('slug') <span class="text-xs text-red-500 font-medium italic">{{ $message }}</span> @enderror
                    </div>

                    {{-- Tipo de campo --}}
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.attributes.type_label') }} <span class="text-red-500">*</span></label>
                        <select wire:model="type"
                            ...
                            <option value="text">{{ __('cms.attributes.type_text') }}</option>
                            <option value="number">{{ __('cms.attributes.type_number') }}</option>
                            <option value="select">{{ __('cms.attributes.type_select') }}</option>
                            <option value="boolean">{{ __('cms.attributes.type_boolean') }}</option>
                        </select>
                    ...
                    {{-- Unidad --}}
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.attributes.unit_label') }}</label>
                        <input type="text" wire:model="unit" placeholder="{{ __('cms.attributes.unit_placeholder') }}"
                               class="w-full px-3 py-2.5 bg-slate-50 border border-slate-100 text-sm text-slate-700 rounded-lg focus:outline-none focus:border-primary transition-colors placeholder-slate-300" />
                    </div>

                    {{-- Opciones (solo select) --}}
                    <div class="space-y-1.5" wire:key="options-block-{{ $type }}">
                        <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.attributes.options_label') }}</label>
                        <textarea wire:model="options" rows="4"
                                  ...
                                  placeholder="{{ __('cms.attributes.options_placeholder') }}" {{ $type === 'select' ? '' : 'disabled' }}></textarea>
                    </div>

                    {{-- Descripción --}}
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.attributes.description_label') }}</label>
                        <textarea wire:model="description" rows="3"
                                  class="w-full px-3 py-2 bg-slate-50 border border-slate-100 text-sm text-slate-700 rounded-lg focus:outline-none focus:border-primary transition-colors placeholder-slate-300 resize-none"
                                  placeholder="{{ __('cms.attributes.description_placeholder') }}"></textarea>
                    </div>

                    {{-- Flags --}}
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <label class="flex items-center gap-2 text-sm text-slate-600 cursor-pointer">
                            <input type="checkbox" wire:model="is_required" class="rounded border-slate-300 text-primary focus:ring-primary">
                            {{ __('cms.attributes.is_required_label') }}
                        </label>
                        <label class="flex items-center gap-2 text-sm text-slate-600 cursor-pointer">
                            <input type="checkbox" wire:model="is_filterable" class="rounded border-slate-300 text-primary focus:ring-primary">
                            {{ __('cms.attributes.is_filterable_label') }}
                        </label>
                    </div>
                ...
                <div class="p-6 border-t border-slate-50 bg-slate-50/30 flex justify-end gap-3">
                    ...
                    <button type="submit" wire:loading.attr="disabled" wire:loading.class="opacity-75 cursor-not-allowed" class="px-6 py-2.5 rounded-lg text-sm font-medium bg-primary hover:bg-[#079d8b] text-white transition-colors border-none cursor-pointer flex items-center justify-center gap-2">
                        ...
                        <span wire:loading.remove wire:target="save">
                            {{ $editingId ? __('cms.general.save') : __('cms.attributes.new_button') }}
                        </span>
                    </button>
                </div>
            </form>
    ...
    <script>
        ...
        title: '{{ __('cms.attributes.delete_title') }}',
                text: '{{ __('cms.general.delete_confirm_text') }}',
                confirmButtonText: '{{ __('cms.attributes.delete_button') }}',
