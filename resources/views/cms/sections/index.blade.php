...
<div class="fixed inset-0 z-[100] flex items-center justify-end">
    ...
    <div class="relative w-full max-w-lg h-full bg-white shadow-xl flex flex-col border-l border-slate-100">
        <div class="p-6 border-b border-slate-50 flex justify-between items-center">
                ...
                <p class="text-xs text-[#c0c1c6]">{{ __('cms.general.id') }} #{{ $editingId }}</p>
            </div>
        ...
        <form wire:submit.prevent="update" class="flex flex-col flex-1 h-full">
            <div class="flex-1 overflow-y-auto p-6 space-y-5">
                ...
                <div class="space-y-1.5">
                    <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.sections.title_label') }}</label>
                ...
                {{-- Contenido --}}
                <div class="space-y-1.5">
                    <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.sections.content_label') }}</label>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.sections.button_label') }}</label>
                        <input type="text" wire:model="name_button" placeholder="{{ __('cms.sections.button_placeholder') }}"
                            class="w-full px-3 py-2 bg-slate-50 border border-slate-100 text-sm text-slate-700 rounded-lg focus:outline-none focus:border-primary transition-colors placeholder-slate-300" />
                    </div>
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.sections.url_label') }}</label>
                ...
                {{-- Imagen principal --}}
                <div class="space-y-1.5">
                    <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.sections.main_image_label') }}</label>
                ...
                @if(count($photos) > 0)
                <div class="space-y-2">
                    <label class="text-xs font-semibold text-[#c0c1c6] uppercase tracking-wider">{{ __('cms.sections.linked_photos') }}</label>
                    <div class="grid grid-cols-3 gap-2">
                        ...
                        <div class="relative group bg-slate-50 rounded-lg border border-slate-100 p-2 flex flex-col items-center">
                            ...
                            <button type="button" wire:click="removePhoto('{{ $photo['name'] }}')" class="text-[10px] text-red-500 hover:text-red-700 font-medium">{{ __('cms.sections.delete_photo') }}</button>
                        </div>
                        @endforeach
                    </div>
                </div>
