<div class="min-h-screen pb-12 bg-soft relative">
    ...
    <div class="relative z-10 p-6 space-y-6">
        ...
        <x-ui-section-header :module-id="\App\Models\Module::CATALOG" :submodule-id="\App\Models\Submodule::PRODUCT_PLATFORMS" :subtitle="__('cms.product_platforms.title')">
            @if(!$showForm)
            <x-slot:action>
                <button wire:click="create" class="rounded-lg bg-primary hover:bg-[#079d8b] text-white px-4 py-2.5 text-sm font-medium transition-colors inline-flex items-center shadow-none border-none cursor-pointer">
                    ...
                    {{ __('cms.product_platforms.new_button') }}
                </button>
            </x-slot:action>
            @endif
        </x-ui-section-header>
        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-[0_1px_2px_0_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="p-4 bg-white border-b border-slate-50 flex flex-col md:flex-row gap-3">
                <div class="relative flex-1">
                    ...
                    <input type="text" wire:model.live="search" placeholder="{{ __('cms.product_platforms.search_placeholder') }}"
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody id="product-platforms-table-body" class="divide-y divide-slate-50 text-sm">
                        @forelse($platforms as $platform)
                        <tr wire:key="line-{{ $platform->id }}" data-id="{{ $platform->id }}" class="sortable-row hover:bg-slate-50/50 transition-colors">
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-center">
                                <span class="text-xs text-slate-500">
                                    {{ $platform->updated_at->format('d/m/Y H:i') }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <x-ui-badge-status :active="$platform->is_active" :active-label="__('cms.general.status_active')" :inactive-label="__('cms.general.status_inactive')" />
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end gap-1">
                                    <x-cms-tooltip text="{{ __('cms.general.edit') }}">
                                        <button wire:click="edit({{ $platform->id }})" class="p-2 text-slate-400 hover:text-primary hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                    ...
                                    <x-cms-tooltip text="{{ __('cms.general.delete') }}">
                                        <button onclick="deleteLine({{ $platform->id }})" class="p-2 text-slate-400 hover:text-red-500 hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        <tr>
                            <td colspan="4">
                                <x-ui-empty-state icon="folder" :title="__('cms.product_platforms.no_results')" />
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="max-w-4xl mx-auto bg-white rounded-xl border border-slate-100 shadow-[0_1px_3px_0_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="p-6 border-b border-slate-50">
                <h2 class="text-lg font-bold text-heading">
                    {{ $editingId ? __('cms.product_platforms.edit_title') : __('cms.product_platforms.new_title') }}
                </h2>
                <p class="text-xs text-slate-400 mt-1">{{ __('cms.product_platforms.subtitle') }}</p>
            </div>
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-1.5">
                            <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ __('cms.product_platforms.name_label') }} <span class="text-red-500">*</span></label>
                            <input type="text" wire:model="name" placeholder="{{ __('cms.product_platforms.name_placeholder') }}"
                        ...
                        <div class="space-y-1.5">
                            <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ __('cms.product_platforms.slug_label') }}</label>
                            <input type="text" wire:model="slug" placeholder="{{ __('cms.product_platforms.slug_placeholder') }}"
                                   class="w-full px-3 py-2.5 bg-white border border-line text-sm text-slate-700 rounded-xl focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-colors placeholder-slate-300" />
                            @error('slug') <span class="text-xs text-red-500 font-medium italic">{{ $message }}</span> @enderror
                            <p class="text-xs text-slate-400 italic mt-1">{{ __('cms.categories.slug_helper') }}</p>
                        </div>
                    </div>

                    {{-- Textareas a ancho completo --}}
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ __('cms.general.description_label') }}</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ __('cms.general.seo_description_label') }}</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ __('cms.general.seo_keywords_label') }}</label>
                ...
                <div class="p-6 border-t border-slate-50 bg-slate-50/30 flex justify-end gap-3">
                    ...
                    <button type="submit" wire:loading.attr="disabled" wire:loading.class="opacity-75 cursor-not-allowed" class="px-6 py-2.5 rounded-lg text-sm font-medium bg-primary hover:bg-[#079d8b] text-white transition-colors border-none cursor-pointer flex items-center justify-center gap-2">
                        ...
                        <span wire:loading.remove wire:target="save">
                            {{ $editingId ? __('cms.general.save') : __('cms.product_platforms.new_button') }}
                        </span>
                        <span wire:loading wire:target="save">
                            {{ $editingId ? __('cms.general.save') : __('cms.product_platforms.new_button') }}
                        </span>
    ...
    <script>
    function deletePlatform(lineId) {
        window.confirmDelete({
        title: '{{ __('cms.product_platforms.delete_title') }}',
                text: '{{ __('cms.product_platforms.delete_warning') }}',
                confirmButtonText: '{{ __('cms.product_platforms.delete_button') }}',
        ...
        const tbody = document.getElementById('product-platforms-table-body');
        if (!tbody) return;
