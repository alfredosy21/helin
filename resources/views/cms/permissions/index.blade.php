<div class="min-h-screen pb-12 bg-soft relative">
    <div class="relative z-10 p-6 space-y-6">
            ...
            <div class="bg-white rounded-xl border border-slate-100 shadow-[0_1px_2px_0_rgba(0,0,0,0.02)] overflow-hidden">
                <div class="p-6 border-b border-slate-50 bg-white">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-4">
                            <div class="w-10 h-10 bg-slate-50 border border-slate-100 rounded-xl flex items-center justify-center text-body shadow-sm">
                                <x-ui-icon name="{{ $module['class'] }}" class="w-5 h-5 stroke-[1.75]" />
                            </div>

                            <div>
                                <h3 class="text-base font-bold text-heading">{{ $module['name'] }}</h3>
                        ...
                        <div class="flex items-center space-x-3">
                            <span class="text-xs font-semibold uppercase tracking-wider {{ $module['status'] == 1 ? 'text-primary' : 'text-body' }}">
                ...
                <div class="p-6 bg-slate-50/50 border-t border-slate-50">
                    <div class="flex items-center justify-between mb-4">
                        <h4 class="text-xs font-bold text-body uppercase tracking-wider">{{ __('cms.permissions.submodules_actions') }}</h4>
                        <div class="flex items-center gap-2">
                            <button wire:click="toggleAllSubmodules(1, {{ $module['module_id'] }})"
                                    class="text-xs px-3 py-1 bg-primary-500/10 text-primary-700 border border-primary-500/20 rounded-full hover:bg-primary-500/20 transition-colors font-medium">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        <div class="flex items-center justify-between p-4 bg-white rounded-xl border border-slate-100 shadow-sm">
                            <div class="flex items-center space-x-3">
                                <div class="w-6 h-6 bg-slate-50 rounded-lg flex items-center justify-center text-body border border-slate-100">
                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6"/>
                                    </svg>
                                </div>

                                <span class="text-sm font-medium text-body">{{ $submodule['name'] }}</span>
                            </div>
                ...
                @else
                <div class="p-6 text-center text-body bg-slate-50/50">
        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-[0_1px_2px_0_rgba(0,0,0,0.02)] p-12 text-center">
            ...
            <h3 class="text-lg font-bold text-heading mb-1">{{ __('cms.permissions.no_permissions_title') }}</h3>
            <p class="text-[13px] text-body mb-6">{{ __('cms.permissions.no_permissions_text') }}</p>
        </div>
        @endif

        <div class="relative z-10">
            <a href="{{ route('cms.roles') }}" wire:navigate class="inline-flex items-center px-5 py-2.5 bg-white text-body rounded-xl text-sm font-semibold hover:bg-slate-50 transition-all duration-200 shadow-sm border border-slate-200/80">
