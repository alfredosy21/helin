    ...
    <div class="relative z-10 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-6">
        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-[0_1px_2px_0_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="p-3 sm:p-4 bg-white border-b border-slate-50 flex flex-col sm:flex-row gap-2 sm:gap-3">
                ...
                <select wire:model.live="stateFilter" class="cms-perpage-select bg-slate-50 border border-slate-100 rounded-lg px-3 py-1.5 pr-8 text-[13px] text-body focus:outline-none focus:border-primary transition-colors appearance-none cursor-pointer">
                ...
                <table class="w-full table-fixed text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/60 text-[10px] font-semibold text-body uppercase tracking-wider border-b border-slate-100">
                            <th class="px-4 py-2.5">{{ __('cms.whatsapp_numbers.phone_number') }}</th>
                            <th class="px-4 py-2.5">{{ __('cms.whatsapp_numbers.executive') }}</th>
                            <th class="px-4 py-2.5">{{ __('cms.whatsapp_numbers.state') }}</th>
                    ...
                    <tbody class="divide-y divide-slate-100 text-[13px]">
                        ...
                        <tr wire:key="whatsapp-{{ $whatsappNumber->id }}" class="hover:bg-slate-50/70 transition-colors duration-150">
                            <td class="px-4 py-2.5">
                                <div class="flex items-center gap-2">
                                    <i class="fab fa-whatsapp text-[#25D366] text-lg"></i>
                                    <div>
                                        <div class="text-body truncate">{{ $whatsappNumber->formatted_number }}</div>
                                        @if($whatsappNumber->description)
                                            <div class="text-[13px] text-body truncate">{{ $whatsappNumber->description }}</div>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-2.5">
                                <span class="text-[13px] text-body">{{ $whatsappNumber->executive_name ?? '—' }}</span>
                            </td>
                            <td class="px-4 py-2.5">
                                <div class="flex flex-wrap gap-1">
                                    @forelse($whatsappNumber->states as $state)
                                        <span class="inline-block px-2 py-0.5 text-[11px] rounded-full bg-slate-100 text-body">{{ $state->name }}</span>
                                    @empty
                            ...
                            <td class="px-4 py-2.5 text-right">
                                <div class="flex justify-end gap-1">
                                    <x-cms-tooltip text="{{ __('cms.general.edit') }}">
                                        <button wire:click="edit({{ $whatsappNumber->id }})" class="p-2 text-body hover:text-primary hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                    ...
                                    <x-cms-tooltip text="{{ __('cms.general.delete') }}">
                                        <button onclick="deleteWhatsAppNumber({{ $whatsappNumber->id }})" class="p-2 text-body hover:text-red-500 hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <form wire:submit.prevent="save" class="space-y-4 sm:space-y-5">
            ...
            {{-- Información básica --}}
            <x-ui-form-card title="{{ $editingId ? __('cms.whatsapp_numbers.edit_title') : __('cms.whatsapp_numbers.new_title') }}" description="{{ __('cms.whatsapp_numbers.subtitle') }}" icon="message-circle">
                <div class="space-y-4">
                    <x-ui-toggle wire:model="is_active" :label="__('cms.general.status_active')" />

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                        <div class="space-y-1.5">
                            <label class="text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.whatsapp_numbers.phone_label') }} <span class="text-red-500">*</span></label>
                        ...
                        <div class="space-y-1.5">
                            <label class="text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.whatsapp_numbers.executive_label') }}</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.whatsapp_numbers.state_label') }} <span class="text-red-500">*</span></label>
                        <div class="max-h-48 overflow-y-auto bg-slate-50 border border-slate-100 rounded-lg p-3 space-y-2">
                            @foreach($states as $state)
                                <label class="flex items-center gap-2 cursor-pointer hover:bg-slate-100 rounded px-2 py-1 transition-colors">
                                    ...
                                    <span class="text-[13px] text-body">{{ $state->name }}</span>
                                </label>
                            @endforeach
                        </div>
                        <p class="text-[11px] text-body">Selecciona los estados que cubre este número.</p>
                        @error('state_ids') <span class="text-xs text-red-500 font-medium italic">{{ $message }}</span> @enderror
                    </div>

                    <div class="space-y-1.5">
                        <label class="text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.whatsapp_numbers.description_label') }}</label>
