<div class="min-h-screen pb-12 bg-soft relative">
    ...
    <div class="relative z-10 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-6">
        ...
        <x-ui-section-header :module-id="\App\Models\Module::SETTINGS" :submodule-id="\App\Models\Submodule::CUSTOMER_TYPES" :subtitle="__('cms.customer_types.title')">
            @if(!$showForm)
            <x-slot:action>
                ...
                <table class="w-full table-fixed text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/60 text-[10px] font-semibold text-body uppercase tracking-wider border-b border-slate-100">
                            ...
                            <th class="px-4 py-2.5">{{ __('cms.tables.description') }}</th>
                    ...
                    <tbody id="customer-types-table-body" class="divide-y divide-slate-100 text-[13px]">
                        @forelse($customerTypes as $type)
                        <tr wire:key="customer-type-{{ $type->id }}" data-id="{{ $type->id }}" class="sortable-row hover:bg-slate-50/70 transition-colors duration-150">
                            ...
                            <td class="px-4 py-2.5">
                                <span class="text-[13px] text-body block truncate">{{ $type->description ?? '—' }}</span>
                            </td>
                            <td class="px-4 py-2.5 text-center">
                                <span class="text-[13px] text-body">{{ $type->updated_at->format('d/m/Y H:i') }}</span>
                            </td>
                            <td class="px-4 py-2.5 text-center">
                            ...
                            <td class="px-4 py-2.5 text-right">
                                <div class="flex justify-end gap-1">
                                    <x-cms-tooltip text="{{ __('cms.general.edit') }}">
                                        <button wire:click="edit({{ $type->id }})" class="p-2 text-body hover:text-primary hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                    ...
                                    <x-cms-tooltip text="{{ __('cms.general.delete') }}">
                                        <button onclick="deleteCustomerType({{ $type->id }})" class="p-2 text-body hover:text-red-500 hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                        ...
                        <tr>
                            <td colspan="5" class="px-4 py-8 text-center">
                                <x-ui-empty-state icon="folder" :title="__('cms.customer_types.empty')" />
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <form wire:submit.prevent="save" class="space-y-4 sm:space-y-5">
            ...
            {{-- Información básica --}}
            <x-ui-form-card title="{{ $editingId ? __('cms.customer_types.edit_title') : __('cms.customer_types.new_title') }}" description="{{ __('cms.customer_types.subtitle') }}" icon="users">
                <div class="space-y-4">
                    <x-ui-toggle wire:model="is_active" :label="__('cms.general.status_active')" />

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                        <div class="space-y-1.5">
                            <label class="text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.customer_types.name_label') }} <span class="text-red-500">*</span></label>
                        ...
                        <div class="space-y-1.5">
                            <label class="text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.customer_types.slug_label') }}</label>
                            ...
                            <p class="text-[11px] text-body italic">{{ __('cms.customer_types.slug_helper') }}</p>
                        </div>
                    </div>

                    <div class="space-y-1.5">
                        <label class="text-[11px] font-semibold text-body uppercase tracking-wider">{{ __('cms.customer_types.description_label') }}</label>
