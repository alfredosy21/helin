<div class="min-h-screen pb-12 bg-soft relative">
    ...
    <div class="relative z-10 p-6 space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-2">
            <div>
                <x-cms-breadcrumb :module-id="\App\Models\Module::SETTINGS" :submodule-id="\App\Models\Submodule::CUSTOMER_TYPES" />
                <p class="text-sm text-slate-500 mt-2.5">
                    {{ __('cms.customer_types.title') }}
                </p>
            </div>
            ...
            <button wire:click="create" class="rounded-lg bg-primary hover:bg-[#079d8b] text-white px-4 py-2.5 text-sm font-medium transition-colors inline-flex items-center shadow-none border-none cursor-pointer">
                ...
                {{ __('cms.customer_types.new_button') }}
            </button>
            @endif
        </div>
        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-[0_1px_2px_0_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="p-4 bg-white border-b border-slate-50 flex flex-col md:flex-row gap-3">
                <div class="relative flex-1">
                    ...
                    <input type="text" wire:model.live="search" placeholder="{{ __('cms.customer_types.search_placeholder') }}"
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/70 border-b border-slate-100 text-slate-400 text-xs font-semibold">
                            <th class="px-6 py-3.5">{{ __('cms.tables.name') }}</th>
                            <th class="px-6 py-3.5">{{ __('cms.tables.description') }}</th>
                    ...
                    <tbody id="customer-types-table-body" class="divide-y divide-slate-50 text-sm">
                        @forelse($customerTypes as $type)
                        <tr wire:key="customer-type-{{ $type->id }}" data-id="{{ $type->id }}" class="sortable-row hover:bg-slate-50/50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="font-bold text-heading uppercase tracking-wide text-xs bg-slate-50 border border-slate-100 px-2.5 py-1 rounded">
                                        {{ $type->name }}
                                    </span>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-center">
                                <span class="text-xs text-slate-500">{{ $type->updated_at->format('d/m/Y H:i') }}</span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <x-ui-badge-status :active="$type->is_active" :active-label="__('cms.general.status_active')" :inactive-label="__('cms.general.status_inactive')" />
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end gap-1">
                                    <x-cms-tooltip text="{{ __('cms.general.edit') }}">
                                        <button wire:click="edit({{ $type->id }})" class="p-2 text-slate-400 hover:text-primary hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                                    ...
                                    <x-cms-tooltip text="{{ __('cms.general.delete') }}">
                                        <button onclick="deleteCustomerType({{ $type->id }})" class="p-2 text-slate-400 hover:text-red-500 hover:bg-slate-50 rounded-lg transition-colors border-none bg-transparent cursor-pointer">
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center text-slate-400">
                                    <svg class="w-10 h-10 mb-2 stroke-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z"/></svg>
                                    <p class="text-xs font-medium">{{ __('cms.customer_types.empty') }}</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
        ...
        <div class="max-w-4xl mx-auto bg-white rounded-xl border border-slate-100 shadow-[0_1px_3px_0_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div class="p-6 border-b border-slate-50">
                <h2 class="text-lg font-bold text-heading">
                    {{ $editingId ? __('cms.customer_types.edit_title') : __('cms.customer_types.new_title') }}
                </h2>
                <p class="text-xs text-slate-400 mt-1">{{ __('cms.customer_types.subtitle') }}</p>
            </div>
            ...
            <form wire:submit.prevent="save" class="w-full">
                <div class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-1.5">
                            <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ __('cms.customer_types.name_label') }} <span class="text-red-500">*</span></label>
                            <input type="text" wire:model="name" placeholder="{{ __('cms.customer_types.name_placeholder') }}"
                        ...
                        <div class="space-y-1.5">
                            <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ __('cms.customer_types.slug_label') }}</label>
                            <input type="text" wire:model="slug" placeholder="{{ __('cms.customer_types.slug_placeholder') }}"
                                   class="w-full px-3 py-2.5 bg-white border border-line text-sm text-slate-700 rounded-xl focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-colors placeholder-slate-300" />
                            @error('slug') <span class="text-xs text-red-500 font-medium italic">{{ $message }}</span> @enderror
                            <p class="text-xs text-slate-400 italic mt-1">{{ __('cms.customer_types.slug_helper') }}</p>
                        </div>
                    </div>

                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ __('cms.customer_types.description_label') }}</label>
                ...
                <div class="p-6 border-t border-slate-50 bg-slate-50/30 flex justify-end gap-3">
                    ...
                    <button type="submit" wire:loading.attr="disabled" wire:loading.class="opacity-75 cursor-not-allowed" class="px-6 py-2.5 rounded-lg text-sm font-medium bg-primary hover:bg-[#079d8b] text-white transition-colors border-none cursor-pointer flex items-center justify-center gap-2">
                        ...
                        <span wire:loading.remove wire:target="save">{{ $editingId ? __('cms.general.save') : __('cms.customer_types.new_button') }}</span>
                        <span wire:loading wire:target="save">{{ $editingId ? __('cms.general.save') : __('cms.customer_types.new_button') }}</span>
                    </button>
                </div>
            </form>
        </div>
    ...
    <script>
        function deleteCustomerType(id) {
            window.confirmDelete({
                title: '{{ __('cms.customer_types.delete_title') }}',
                text: '{{ __('cms.general.delete_confirm_text_with_associated') }}',
                confirmButtonText: '{{ __('cms.customer_types.delete_button') }}',
        ...
        (function() {
            ...
            function initSortable() {
                const tbody = document.getElementById('customer-types-table-body');
                ...
                sortableInstance = new Sortable(tbody, {
                    ...
                    onEnd: function() {
                        ...
                        if (component && orderedIds.length > 0) component.updateOrder(orderedIds);
                    }
                });
            }
